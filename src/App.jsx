...
function App() {
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 selection:bg-brand-accent/20 overflow-x-hidden font-sans">
      ...
      {!loading && (
        <>
          <Navbar onCtaClick={() => setIsModalOpen(true)} language={language} setLanguage={setLanguage} />
          <main className="flex flex-col w-full">
            <Hero videoID={language === 'PT' ? 'CJjp8afhPT4' : 'hqwE-6tbSIE'} onCtaClick={() => setIsModalOpen(true)} language={language} backgroundImage="/assets/time.jpg" />
            ...
            <FinalCTA onCtaClick={() => setIsModalOpen(true)} language={language} />
          </main>
          <footer className="py-24 text-center border-t border-zinc-100 px-6 bg-zinc-50">
            <div className="text-brand-accent font-heading font-black italic uppercase tracking-widest text-2xl mb-4">Millennium<span className="text-zinc-900">Football</span></div>
            <p className="text-zinc-400 font-heading text-xs uppercase tracking-[0.3em] font-bold">© 2026 Millennium Football Agency. Todos os direitos reservados.</p>
          </footer>
        </>
      )}
